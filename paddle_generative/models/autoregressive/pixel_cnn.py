...
from paddle_generative import nn as pg_nn
from paddle_generative.models import base


class CausalResidualBlock(nn.Layer):
...
class PixelCNN(base.AutoregressiveModel):
    ...
    def __init__(
        ...
        self._causal_layers = nn.LayerList(
            [
...
def reproduce(
    ...
    from paddle_generative import datasets
    from paddle_generative import models
    from paddle_generative import trainer
    ...
    scheduler = lr.LambdaDecay(optimizer.learning_rate, lr_lambda=lambda _: 0.999977)
    def loss_fn(x, _, preds):
        batch_size = x.shape[0]
        x, preds = paddle.reshape(x, shape=[(batch_size, -1)]), paddle.reshape(x,shape=[batch_size, -1])
