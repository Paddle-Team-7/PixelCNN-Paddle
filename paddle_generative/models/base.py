...
def _default_sample_fn(logits):
    return paddle.bernoulli(logits=logits)


class GenerativeModel(abc.ABC, nn.Layer):
    """Base class inherited by all generative models in paddle-generative.
...
class AutoregressiveModel(GenerativeModel):
    ...
    def _get_conditioned_on(self, n_samples, conditioned_on):
        ...
        if conditioned_on is None:
            ...
            #conditioned_on = (paddle.ones_like(shape) * -1).to(self.device)
            conditioned_on = (paddle.ones_like(shape) * -1)
        else:
        ...
        with paddle.no_grad():
            ...
            for row in range(h):
                for col in range(w):
                    ...
                    conditioned_on[:, :, row, col] = paddle.where(
